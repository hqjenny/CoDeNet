...
#pragma HLS PIPELINE
		scale1[i] = mem[count++];
		//cout<<"scale"<<scale1[i]<<endl;
	}
	for (int i=0; i <rep1; i++) {
#pragma HLS PIPELINE
		bias1[i] = mem[count++];
		//cout<<"bias"<<bias1[i]<<endl;
	}
...
//#pragma HLS ARRAY_PARTITION variable=w_read.data complete dim=1
...
//#pragma HLS INLINE
	T_IN w_read;
//#pragma HLS ARRAY_PARTITION variable=w_read.data complete dim=1
	MultiChanData<KERNEL*KERNEL,W_W*PA*PE> w_write;
	T_IN w_1;
